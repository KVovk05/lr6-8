...
public class ReplenishmentTest {
    ...
    public void testExecuteWithMultipleDepositsValidSelection() {
        ...
        Replenishment command = new Replenishment(myDeposits);
        command.execute();

        assertEquals(1000, myDeposits.get(0).getMyInvestment());
        assertEquals(10500, myDeposits.get(1).getMyInvestment());


        String output = outContent.toString();
        assertTrue(output.contains("Select a deposit to add money:"));
        assertTrue(output.contains("You successfully added money to your deposit"));
    }
    ...
    public void testExecuteWithSingleDeposit() {
        ...
        assertEquals(1300, myDeposits.get(0).getMyInvestment());

        String output = outContent.toString();
        assertTrue(output.contains("How much money do you want to add?"));
        assertTrue(output.contains("You successfully added money to your deposit"));
    }
    ...
    public void testGetDesc() {
        Replenishment command = new Replenishment(myDeposits);
        assertEquals("Replenish my deposit", command.getDesc());
    }
}
